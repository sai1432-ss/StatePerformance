...
import { useCart, useNotification } from '../optimized/SplitContexts.jsx';
...
export const ProductCard = React.memo(({ product }) => {
  ...
  const handleAdd = () => {
    ...
    notifyDispatch({ 
      ...
      payload: { message: `Added ${product.name}!`, type: 'success' } 
    });
    setAdded(true);
  ...
  return (
    <div style={{ border: '1px solid #e2e8f0', padding: '20px', borderRadius: '12px', background: '#fff', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', color: '#333' }}>
      <div>
...
export default React.memo(function ProductListPage() {
  const renderCount = useRef(0);
  renderCount.current += 1;
  return (
    <div>
